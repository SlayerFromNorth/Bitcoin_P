...
ripemd160_GG = gen_ripemd160_with_variable_scope_protector_to_not_pollute_global_namespace()
print(ripemd160_GG(b'hello this is a test').hex())
print("number of bytes in a RIPEMD-160 digest: ", len(ripemd160_GG(b'')))
...
for a in range(50000):
    stringAA = ''.join(random.choices(string.ascii_lowercase, k=100))
    print(stringAA)
    hash = hashlib.new('ripemd160')
    hash.update(stringAA.encode('utf-8'))
    tested = hash.hexdigest()

    tesla = ripemd160_GG(stringAA.encode('utf-8')).hex()
    print(tested)
    print(tesla)
    if tested == tesla:
        success += 1
