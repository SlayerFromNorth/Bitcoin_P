...
sha256_GG = gen_sha256_with_variable_scope_protector_to_not_pollute_global_namespace()
print("verify empty hash:", sha256_GG(b'').hex()) # should be e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
print(sha256_GG(b'here is a random bytes message, cool right?').hex())
print("number of bytes in a sha256 digest: ", len(sha256_GG(b'')))
...
for a in range(50000):
    stringAA = ''.join(random.choices(string.ascii_uppercase, k=100))
    print(stringAA)
    tested = sha256(stringAA.encode('utf-8')).hexdigest()
    tesla = sha256_GG(stringAA.encode('utf-8')).hex()
    print(tested)
    print(tesla)
    if tested == tesla:
        success += 1
